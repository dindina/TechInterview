...
https://leetcode.com/discuss/interview-question/5000969/Amazon-SWE-OA-question/
 */
public class BridgeAndDrivers {

    public static void main(String[] args) {
        int [] weight = new int[]{17, 6, 5, 2, 7, 4, 5, 4};
        int bridgeLimit = 7;
        int n = weight.length;
        int turnBack = 0;
        int i = 0;
        ...
        while (i < n)
            //if (i+1 <n  && weight[i] + weight[i + 1] <= bridgeLimit) {

            if (i <n-1  && weight[i] + weight[i + 1] <= bridgeLimit) {
                i += 2;  // Both cars cross
            } else {
                turnBack++;
                i++;  // Only first car crosses
            }
